...
use super::sha256::{Sha256Algorithm, Sha256Hash};
...
enum Hasher {
    /// Hasher for the overall request id.
    /// This is the only variant that may directly contain a struct.
    RequestId(Sha256Algorithm),

    /// A structure to be included in the hash.
    /// May not contain other structs.
    Struct {
        // A BTreeMap is used here because there is no guarantee
        // that keys are unique. If a duplicate key appears,
    ...
    /// Hasher for a value.
    /// For arrays, each element contributes the hash of its contents
    /// to the hasher of the array.
...
/// The type is serialized into this structure, and then its content is hashed
/// via the corresponding trait implementation.
///
/// Internally, this is a simple state machine with three states:
/// 1. **Root value** — must be a struct.
///    If any other value is serialized, this results in an error.
///    This is indicated by whether `fields` is `Some(_)`.
/// 2. **Struct field processing** — a field value is currently being serialized.
///    In this state, `field_value_hash` must be `Some(_)`.
/// 3. **Finished** — `finish()` has been called.
///    The hasher cannot be reused, and the hash should already have been retrieved.
///
/// Invalid states include:
/// - Serializing a field when `fields` is `None`.
/// - Serializing a non-struct value when `field_value_hash` is `None`.
///
/// Unsupported types always cause failure.
/// For example, `i8` values are not supported and return an `UnsupportedTypeXXX` error.
///
/// Currently supported types are:
/// - Strings and string slices.
/// - Vectors of `u8` (byte strings).
/// - A single struct at the root level (its type name and fields are not validated).
///
/// This also fails on unsupported data structures.
/// For example, a `UnitVariant` other than `Blob`, or a nested struct inside another struct.
///
/// ⚠️ **Note:** This does *not* validate whether a message is semantically valid.
/// The message format may evolve faster than the request ID calculation logic.
...
impl RequestIdSerializer {
    ...
    /// Finalizes the hashing and returns the `RequestId` for the serialized structure.
    ///
    /// This can only be called once because it consumes `self`.
    /// Since this type is not public, this restriction should not be an issue.
    ...
    /// Hashes a single value, returning its SHA-256 hash.
    ///
    /// If another value is already being hashed, this returns `InvalidState`.
    /// Currently, this cannot occur because embedded structures are not allowed,
    /// but the check is kept as a safeguard for future changes.
...
/// custom data format.
impl ser::Serializer for &mut RequestIdSerializer {
    /// The output type produced by this `Serializer` on successful serialization.
    /// For most serializers producing text or binary output, `Ok = ()` is used
    /// and serialization is performed into a buffer or writer contained in the serializer.
    /// Serializers building in-memory data structures may use `Ok` to propagate the data structure.
    type Ok = ();

    /// The error type when some error occurs during serialization.
    type Error = RequestIdError;

    // Associated types for compound structures (sequences, tuples, maps, structs).
    // No additional state is required beyond the serializer itself.
    ...
    ///
    /// Uses LEB128 encoding and then serializes the resulting byte array.
    fn serialize_u64(self, v: u64) -> Result<Self::Ok, Self::Error> {
        // 10 bytes is enough for a 64-bit number in leb128.
        let mut buffer = [0; 10];
        let mut writable = &mut buffer[..];
        let n_bytes =
            leb128::write::unsigned(&mut writable, v).expect("Failed to serialize number.");
    ...
    ///
    /// Treated as an empty value (empty string or blob) in the hash.
    ...
    ///
    /// Treated as the value itself in the hash.
    ...
    /// Begin serializing a sequence of variable length.
    /// Must be followed by calls to `serialize_element` and then `end`.
    fn serialize_seq(self, _len: Option<usize>) -> Result<Self::SerializeSeq, Self::Error> {
        Ok(self)
    }

    /// Begin serializing a tuple (statically sized sequence).
    fn serialize_tuple(self, _len: usize) -> Result<Self::SerializeTuple, Self::Error> {
        Err(RequestIdError::UnsupportedTypeTuple)
    }

    /// Begin serializing a tuple struct like `struct Rgb(u8, u8, u8)`.
    /// Must be followed by calls to `serialize_element` and then `end`.
    ...
    /// Begin serializing a tuple variant like `E::T` in `enum E { T(u8, u8) }`.
    /// Must be followed by calls to `serialize_element` and then `end`.
    ...
    /// Begin serializing a struct like `struct Rgb { r: u8, g: u8, b: u8 }`.
    /// Must be followed by calls to `serialize_element` and then `end`.
    fn serialize_struct(
        self,
        _name: &'static str,
    ...
    /// Begin serializing a struct variant like `E::S` in `enum E { S { r: u8, g: u8, b: u8 } }`.
    /// Must be followed by calls to `serialize_element` and then `end`.
    ...
    /// Indicates whether this serializer produces human-readable output.
    fn is_human_readable(&self) -> bool {
        false
    }
}

// The following 7 impls handles serialization of sequences (compound types).
// Serialization begins with `serialize_seq` on the serializer, followed by
// zero or more calls to `serialize_element`, and finished by a call to `end`.
//
// note: serialization of tuple, tuple variants, tuple struct, maps are not supported by RequestIdSerializer.
// All attempts to serialize fields of these types will return an error.
impl ser::SerializeSeq for &mut RequestIdSerializer {
...
impl ser::SerializeTuple for &mut RequestIdSerializer {
    type Ok = ();
    type Error = RequestIdError;
...
impl ser::SerializeTupleStruct for &mut RequestIdSerializer {
    type Ok = ();
    type Error = RequestIdError;
...
// the `}`.
impl ser::SerializeTupleVariant for &mut RequestIdSerializer {
    type Ok = ();
    type Error = RequestIdError;
...
impl ser::SerializeMap for &mut RequestIdSerializer {
    type Ok = ();
    type Error = RequestIdError;
...
// constant strings.
impl ser::SerializeStruct for &mut RequestIdSerializer {
    type Ok = ();
    type Error = RequestIdError;
...
impl ser::SerializeStructVariant for &mut RequestIdSerializer {
    type Ok = ();
    type Error = RequestIdError;
