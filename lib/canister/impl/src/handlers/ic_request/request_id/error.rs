/// Error type for RequestId calculation.
use thiserror::Error;

/// Errors that can occur when reading a RequestId from a string.
/// Note: this is not the same as deserialization.
#[derive(Error, Debug)]
...
/// Errors that can occur during RequestId calculation.
/// Since serde is used to serialize a data type into a hash, this error type
/// must implement traits expected by serde, such as [Display].
